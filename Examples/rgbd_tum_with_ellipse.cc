...
int main(int argc, char **argv) {
    ...
    if (argc != 8) {
        cerr << endl
             << "Usage:\n"
                ...
                "      vocabulary_file\n"
                "      camera_file\n"
                "      path_to_image_sequence (.txt file listing the images or "
                "a folder with rgb.txt or 'webcam_id')\n"
                "      path_to_association \n"
                "      detections_file (.json file with detections)\n"
                "      relocalization_mode ('points', 'objects' or "
                "'points+objects' currently on points used)\n"
                "      output_name \n";
        return 1;
    }
    ...
    if (extension == "json") {  // load from external detections file
        detector =
            std::make_shared<ORB_SLAM2::DetectionsFromFile>(detections_file);
    ...
    ORB_SLAM2::System SLAM(vocabulary_file, parameters_file,
                           ORB_SLAM2::System::RGBD, true, false);
    ...
    while (1) {
        ...
        filename = path_to_images + '/' +
                   vstrImageFilenamesRGB[ni];  // path_to_images +
                                               // vstrImageFilenames[ni];
        // std::cout<<"filename:"<<filename<<std::endl;
        imRGB =
            cv::imread(filename, cv::IMREAD_UNCHANGED);  // read image from disk
        imD = cv::imread(path_to_images + '/' + vstrImageFilenamesD[ni],
                         cv::IMREAD_UNCHANGED);
        ...
        if (detector) {
            if (detect_from_file)
                detections =
                    detector->detect(filename);  // from detections file
            else
        ...
        cv::Mat m =
            SLAM.TrackRGBD(imRGB, imD, tframe, detections, false, false);
    ...
    SLAM.SaveMapObjectsOBJ(output_folder + "map_objects_" + output_name +
                           ".obj");
    SLAM.SaveMapObjectsTXT(output_folder + "map_objects_" + output_name +
                           ".txt");
    std::cout << "\n";
